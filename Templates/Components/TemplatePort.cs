...
namespace Incas.Templates.Components
{
    public class TemplatePort
    {
        ...
        {
            Data.SourceTemplate = temp;
            using (Tag tag = new())
            {
                if (applyParent)
                {
                    Data.Tags = tag.GetAllTagsByTemplate(Data.SourceTemplate.id, Data.SourceTemplate.parent);
                }
                else
                {
                    Data.Tags = tag.GetAllTagsByTemplate(Data.SourceTemplate.id);
                }
            }
            ...
            Data.SourceTemplate.id = 0;
            Data.SourceTemplate.parent = "";
        ...
        {
            string zipFolder = $"{folder}\\{Data.SourceTemplate.name}";
            Directory.CreateDirectory(zipFolder);
            File.WriteAllText($"{zipFolder}\\{dataName}", JsonConvert.SerializeObject(Data));
            ...
            using (ZipArchive zip = ZipFile.Open(file, ZipArchiveMode.Read))
            {
                ...
                if (entry != null)
                {
                    ...
                    using (Stream entryStream = entry.Open())
                    {
                        ...
                        {
                            Data = JsonConvert.DeserializeObject<TemplateData>(reader.ReadToEnd());
                        }
                    }
                }
            }
                    ...
                    if (entry.Name.EndsWith(".docx"))
                    {
                        if (File.Exists(ProgramState.GetFullnameOfWordFile(entry.Name))) // если файл с таким именем есть в папке Word
                        {
                                ...
                                $"Использовать его или переименовать предлагаемый файл из импортированного шаблона?", "Использовать старый шаблон?", "Использовать старый", "Переименовать предлагаемый") == DialogStatus.No)
                            {
                                ...
                                Data.SourceTemplate.path = newpath;
